...
def color_segment_pick(img):
    ...
    print("color",des)
    if(des==0):
          #mask_1 = cv2.inRange(hsv_roi, np.array([36, 97, 66]), np.array([159, 255, 255]))
          mask_1 = cv2.inRange(hsv_roi, np.array([70, 200, 92]), np.array([120, 255, 243]))
          #mask_1 = cv2.inRange(hsv_roi, np.array([95, 93, 80]), np.array([133, 255, 255]))
    elif(des==1):
          mask_1 = cv2.inRange(hsv_roi, np.array([33, 92, 0]), np.array([93, 255, 255])) #
    mask1 = mask_1 
    kern_dilate = np.ones((8,8),np.uint8)
    kern_erode  = np.ones((3,3),np.uint8)
    mask= cv2.erode(mask1,kern_erode)      #Eroding
    mask=cv2.dilate(mask1,kern_dilate)     #Dilating
